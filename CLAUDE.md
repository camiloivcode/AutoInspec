...
Vite + React 18 + TypeScript + TailwindCSS. Two pages (`GenerarPDF`, `History`) lazy-loaded in `App.tsx`, inside a `Layout` shell (fixed rail ≥`md`, `BottomNav` below it). The CRUD-era pages, `services/api.ts` and `types/index.ts` were deleted in `6aa69d2` — nothing under `src/` talks to the legacy CRUD endpoints anymore.
