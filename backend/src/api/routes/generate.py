...
def _validate_generation_request(driver_name: str, plate: str, images: List[UploadFile], positions: List[str]) -> None:
...
async def _save_uploads(images: List[UploadFile], positions: List[str], work_dir: str) -> tuple[dict[int, list[str]], list[str]]:
    ...
    return image_map, saved_paths


def _render_and_respond(driver_name: str, plate: str, image_map: dict[int, list[str]]) -> FileResponse:
...
async def generate_pdf(
    ...
    _validate_generation_request(driver_name, plate, images, positions)
    ...
    try:
        image_map, _saved_paths = await _save_uploads(images, positions, work_dir)
        return _render_and_respond(driver_name, plate, image_map)
    except Exception as e:
...
async def generate_pdf_auto(
    ...
    try:
        image_map, saved_paths = await _save_uploads(images, positions, work_dir)

        if suggested_positions is not None:
            # Must run before the `finally` block below deletes work_dir --
            # record_batch copies these photos out for the feedback loop.
